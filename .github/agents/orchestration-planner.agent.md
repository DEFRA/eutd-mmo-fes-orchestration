...
Orchestration** service (Node.js/TypeScript/Hapi.js).
...
- Produce complete implementation plans for Orchestration work (routes/controllers, dual-strategy
  authentication (JWT + Basic FES API), document-ownership validation (`withDocumentLegitimatelyOwned`), dual
  storage (Redis sessions + MongoDB/Mongoose documents), Joi validation with custom `failAction`, GOV.UK
  Notify, Azure Blob and Event Hubs integrations).
...
5. Define impacted files/components and expected changes at a high level — including whether the change
   touches the dual-auth strategies (`src/helpers/auth.ts`), document-ownership validation
   (`src/helpers/withDocumentLegitimatelyOwned.ts`), session storage (`src/session_store/`), the Mongoose
   persistence schemas (`src/persistence/schema/`), Joi validation, and centralised config
   (`src/applicationConfig.ts`) — and which of these must be updated together.
6. Define the validation strategy: Jest unit/integration tests (`.jest.spec.ts` files, MongoDB Memory
   Server, mocked external services), coverage targets, and the `npm run build` / `npm test` (`npm run
   test:ci` in CI) / `npm run lint` commands, noting which steps your research validated and citing the
   sources.
7. Identify risks, regressions, and mitigation steps — especially around auth, document-ownership and
   dual-storage correctness.
