...
- **Invoked standalone without a plan?** Apply the framework's triage:
  ...
  - **Standard** (a normal feature/fix with no new architecture, auth/ownership change, or security
    ...
    and obtain user approval before implementing. Do **not** invoke the heavyweight Planner for this.
  - **Complex** (auth/ownership/validation architecture change, session/document storage architecture
    change, a new external integration, a security surface) — delegate planning to the
    [Planner - Orchestration Service](orchestration-planner.agent.md), do **not** author it yourself, then
...
- **Manual override.** If the user explicitly forces a gear ("treat this as trivial", "just a lightweight
  ...
  genuinely touches architecture, auth/ownership, external integrations, security or data correctness.
...
await notifyClient.sendEmail(appConfig.successTemplateId, email, {
  personalisation: {
    documentNumber,
...
describe('CatchCertificateController', () => {
  ...
  it('should return 401 for unauthenticated user', async () => {
    ...
    await expect(controller.getCatchCertificate(mockRequest)).rejects.toThrow(
      'Unauthorized',
    );
  });
});
...
- **Testing & coverage**: Write tests alongside code. Tiered targets — **≥90% global, ≥95% core business logic, 100% error-handling and security-critical paths**. Never drop below the project or SonarCloud baseline. Test behaviour, not implementation. Mock external dependencies (APIs, DB, Redis, Notify, Blob Storage).
