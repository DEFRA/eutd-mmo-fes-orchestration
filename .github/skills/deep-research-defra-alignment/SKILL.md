...
description: "Do thorough, risk-scoped research in the open and align findings to the DEFRA standards precedence (DEFRA > GDS > community) for the MMO FES Orchestration service. Use for the single, risk-scoped Research (§4.2) stage of the working framework — validating APIs, libraries, patterns, security and policy against DEFRA/GDS and framework guidance, and citing sources before a plan is approved or implemented."
argument-hint: "e.g. 'validate the JWT issuer-validation approach the planner flagged' or 'research Redis session TTL vs Cosmos document expiry'"
license: OGL-UK-3.0
metadata:
  author: mmo-fes
  version: "1.0"
...
- **Planner - Orchestration Service** runs this single research pass for **Complex** work and cites sources
  directly in its plan.
- **Developer - Orchestration Service** runs this same single pass for **Standard** work (or when invoked
...
Match effort to consequence. Go deeper the closer a step is to: **security / secrets / PII**,
**authentication and authorisation** (dual-strategy JWT + Basic FES API, document-ownership validation),
**data correctness** (session/document state, request validation), **persistence** (Redis sessions,
MongoDB/Mongoose documents), **external integrations** (Reference/Consolidation services, GOV.UK Notify,
Azure Blob Storage, Event Hubs protective monitoring), or a **version-sensitive API/library**. A cosmetic or
...
State the concrete decision to be made, the constraint it touches (auth/authorisation, data correctness,
encryption in transit, no secrets, no PII in logs, structured logging, test coverage, dependency policy), and
what a good answer must let you decide.
...
- Search **authoritative, current** sources: DEFRA & GOV.UK standards and service manuals, OWASP, the
  framework/library's own docs (Hapi, `@hapi/basic`, `joi`, `jsonwebtoken`/JWT, Mongoose, `ioredis`,
  `notifications-node-client` (GOV.UK Notify), `@azure/storage-blob`, `@azure/event-hubs`), and
  `mmo-shared-reference-data`. Prefer primary sources over blog posts.
...
- [ ] **No secrets in code** — configuration and credentials from environment/config
      (`src/applicationConfig.ts`) only; never `process.env` scattered through code.
- [ ] **Authentication & authorisation** — dual-strategy auth (JWT + Basic FES API) is enforced via
      `defineAuthStrategies()`, and document ownership is validated with `withDocumentLegitimatelyOwned`
      before any document operation.
- [ ] **Input validation** — all request inputs validated and sanitised with `joi`, with the custom
      `failAction` handling HTML vs JSON clients (`acceptsHtml`).
- [ ] **No PII in logs** — names, addresses, emails, phone numbers, NI numbers, bank details, tokens.
- [ ] **Structured logging** — structured JSON logging with correlation/`requestId` propagated end-to-end.
- [ ] **Data correctness** — session (Redis) and document (MongoDB/Mongoose) state stay consistent; document
      numbers are normalised (`.toUpperCase()`); colon-delimited session keys are used.
- [ ] **Testing** — change is testable via `.jest.spec.ts` tests (MongoDB Memory Server, mocked external
      services) and keeps tiered coverage (≥90% global, ≥95% core logic, 100% error-handling/security paths)
      without dropping below the SonarCloud baseline.
...
**blocked**); send **revise/blocked** items back to the **Planner - Orchestration Service** rather than
