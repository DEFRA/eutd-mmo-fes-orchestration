...
describe('Certificates routes', () => {
  ...
  describe('GET /v1/certificate/catchCertificate/pre-submit', () => {
    const request: any = {
      method: 'GET',
      url: '/v1/certificate/catchCertificate/pre-submit',
      app: {
        claims: {
          sub: 'Bob',
    ...
    let mockGetCatchCertificatePreSubmit: jest.SpyInstance;
    let mockValidateDocumentOwnership: jest.SpyInstance;
    let mockLogError: jest.SpyInstance;

    beforeAll(() => {
      mockGetCatchCertificatePreSubmit = jest.spyOn(CertificateController, 'getCatchCertificatePreSubmit');
      mockValidateDocumentOwnership = jest.spyOn(DocumentOwnershipValidator, 'validateDocumentOwnership');
      mockLogError = jest.spyOn(logger, 'error');
    });
    ...
    it('will return 404 when no pre-submit bundle is found', async () => {
      mockGetCatchCertificatePreSubmit.mockResolvedValue(null);

      const response = await server.inject(request);

      expect(mockGetCatchCertificatePreSubmit).toHaveBeenCalled();
      expect(response.statusCode).toBe(404);
    });

    it('will return 200 with pre-submit bundle', async () => {
      mockGetCatchCertificatePreSubmit.mockResolvedValue({
        summary: { documentNumber: 'GBR-2020-CC-0E42C2DA5', status: 'DRAFT' },
        completeness: { requiredSections: 7, completedSections: 7 }
      });
    ...
    it('will return 403 if the user does not own the document', async () => {
      ...
      expect(mockGetCatchCertificatePreSubmit).not.toHaveBeenCalled();
      expect(response.statusCode).toBe(403);
    });

    it('will log and return 500 when the pre-submit handler throws', async () => {
      const e = new Error('pre-submit failed');
      mockGetCatchCertificatePreSubmit.mockRejectedValue(e);

      const response = await server.inject(request);

      expect(response.statusCode).toBe(500);
      expect(mockLogError).toHaveBeenCalledWith(`[GET-CC-PRE-SUBMIT][ERROR][${e.stack || e}]`);
    });
  });
