...
describe("errorExtractor", () => {
  ...
  it("buildErrorObject() should handle containerNumbers array.unique and mark both positions", () => {
    const data = {
      details: [{
        type: "array.unique",
        path: ["containerNumbers", 1],
        context: { pos: 1, dupePos: 0, value: "ABCU1234567", dupeValue: "ABCU1234567" }
      }],
    ...
    expect(result).toEqual({
      ...
      "containerNumbers.0": "error.containerNumbers.array.unique"
    });
  });

  it("buildErrorObject() should handle containerNumbers array.unique for non-zero duplicate position", () => {
    const data = {
      details: [{
        type: "array.unique",
        path: ["containerNumbers", 2],
        context: { pos: 2, dupePos: 0, value: "ABCU1234567", dupeValue: "ABCU1234567" }
      }],
      _original: { vehicle: "train", containerNumbers: ["ABCU1234567", "BCDU1234567", "ABCU1234567"] }
    };
  ...
  it("buildNonJsErrorObject() should handle containerNumbers array.unique and mark both positions", () => {
    const data = {
      details: [{
        type: "array.unique",
        path: ["containerNumbers", 1],
    ...
    const result = buildNonJsErrorObject(data, null);
