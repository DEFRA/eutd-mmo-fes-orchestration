...
describe('checkDocument', () => {
  ...
  it('will return false when draft threshold is reached', async () => {
    ...
    expect(mockUserCanCreateDraft).toHaveBeenCalledWith('Bob', 'storageNotes', defaultContact);
    mockUserCanCreateDraft.mockRestore();
  });
});
...
describe('cloneStorageDocument', () => {
  ...
  it('resolves getDocument and getUniqueDocumentNumber in parallel (Promise.all)', async () => {
    const getDocumentSpy = jest.spyOn(StorageDocumentService, 'getDocument');

    const result = await StorageDocumentService.cloneStorageDocument(
      originalDocNumber, 'Bob', defaultContact, requestByAdmin, voidOriginal
    );

    expect(getDocumentSpy).toHaveBeenCalledTimes(1);
    expect(getDocumentSpy).toHaveBeenCalledWith(originalDocNumber, 'Bob', defaultContact);
    expect(result).toBe(cloneDocNumber);
...
describe('getCompletedDocuments — lean results', () => {
  it('returns plain objects with no Mongoose instance methods (.lean())', async () => {
    await createDocument('Bob', 'COMPLETE', 'SD-LEAN-001');

    const results = await StorageDocumentService.getCompletedDocuments(defaultUser, defaultContact, 10, 1);
  ...
  it('returns only documents belonging to the requesting user', async () => {
    await createDocument('Bob', 'COMPLETE', 'SD-OWN-001');
    await createDocument('OtherUser', 'COMPLETE', 'SD-OWN-002');

    const results = await StorageDocumentService.getCompletedDocuments(defaultUser, defaultContact, 10, 1);

    expect(results.every((d: any) => d.documentNumber !== 'SD-OWN-002')).toBe(true);
  });

  it('respects pagination — skip and limit', async () => {
    await createDocument('Bob', 'COMPLETE', 'SD-PAGE-001', new Date('2024-01-01'));
    await createDocument('Bob', 'COMPLETE', 'SD-PAGE-002', new Date('2024-06-01'));
    await createDocument('Bob', 'COMPLETE', 'SD-PAGE-003', new Date('2024-12-01'));

    const page1 = await StorageDocumentService.getCompletedDocuments(defaultUser, defaultContact, 2, 1);
    const page2 = await StorageDocumentService.getCompletedDocuments(defaultUser, defaultContact, 2, 2);
...
describe('getAllStorageDocsForUser — lean results', () => {
  it('returns plain objects with no Mongoose instance methods (.lean())', async () => {
    await createDocument('Bob', 'COMPLETE', 'SD-ALL-LEAN-001');

    const results = await StorageDocumentService.getAllStorageDocsForUser(defaultUser, defaultContact);

    expect(results.length).toBeGreaterThan(0);
    expect(typeof (results[0] as any).save).toBe('undefined');
  });

  it('excludes VOID documents', async () => {
    await createDocument('Bob', 'COMPLETE', 'SD-ALL-001');
    await createDocument('Bob', 'VOID', 'SD-ALL-VOID-001');

    const results = await StorageDocumentService.getAllStorageDocsForUser(defaultUser, defaultContact);
    const docNumbers = results.map((d: any) => d.documentNumber);

    expect(docNumbers).toContain('SD-ALL-001');
    expect(docNumbers).not.toContain('SD-ALL-VOID-001');
  });
});

describe('getDraftDocumentHeaders — lean + sort', () => {
  it('returns plain objects with no Mongoose instance methods (.lean())', async () => {
    await createDocument('Bob', 'DRAFT', 'SD-HDR-LEAN-001');

    const results = await StorageDocumentService.getDraftDocumentHeaders(defaultUser, defaultContact);
  ...
  it('returns headers sorted by createdAt descending', async () => {
    await createDocument('Bob', 'DRAFT', 'SD-SORT-OLD', new Date('2024-01-01'));
    await createDocument('Bob', 'DRAFT', 'SD-SORT-NEW', new Date('2024-12-01'));
...
describe('completeDraft — projection: { _id: 1 }', () => {
  it('sets status to COMPLETE and stores documentUri', async () => {
    await createDocument('Bob', 'DRAFT', 'SD-COMP-001');

    await StorageDocumentService.completeDraft('SD-COMP-001', 'http://example.com/sd.pdf', 'bob@test.com');

    const doc = await StorageDocumentModel.findOne({ documentNumber: 'SD-COMP-001' }).lean();
    expect((doc as any).status).toBe('COMPLETE');
    expect((doc as any).documentUri).toBe('http://example.com/sd.pdf');
  });

  it('does not overwrite a document already in COMPLETE status', async () => {
    await createDocument('Bob', 'COMPLETE', 'SD-COMP-002');
    await StorageDocumentService.completeDraft('SD-COMP-002', 'http://new.com/sd.pdf', 'bob@test.com');

    const doc = await StorageDocumentModel.findOne({ documentNumber: 'SD-COMP-002' }).lean();
    // completeDraft uses { $ne: 'COMPLETE' } filter so it will NOT update an already-COMPLETE doc
    expect((doc as any).documentUri).not.toBe('http://new.com/sd.pdf');
  });
});

describe('deleteDraft — projection: { _id: 1 }', () => {
  it('deletes the draft and removes it from DB', async () => {
    await createDocument('Bob', 'DRAFT', 'SD-DEL-001');

    await StorageDocumentService.deleteDraft(defaultUser, 'SD-DEL-001', defaultContact);

    const remaining = await StorageDocumentModel.findOne({ documentNumber: 'SD-DEL-001' });
    expect(remaining).toBeNull();
  });

  it('does nothing when no matching draft exists', async () => {
    const before = await StorageDocumentModel.countDocuments();
    await StorageDocumentService.deleteDraft(defaultUser, 'SD-NONEXISTENT', defaultContact);
    expect(await StorageDocumentModel.countDocuments()).toBe(before);
  });
});

describe('cloneStorageDocument — Promise.all parallelisation verification', () => {
  const originalDocNumber = 'SD-PAR-ORIG-001';
  const cloneDocNum = 'SD-PAR-CLONE-001';

  const originalDoc: StorageDocument = {
    createdAt: new Date().toISOString(),
    createdBy: defaultUser,
    createdByEmail: 'bob@test.com',
    status: 'DRAFT',
    documentNumber: originalDocNumber,
    requestByAdmin: false,
  ...
  beforeEach(async () => {
    ...
    await new StorageDocumentModel(originalDoc).save();
  });

  it('getUniqueDocumentNumber and getDocument are both called exactly once', async () => {
    const getDocSpy = jest.spyOn(StorageDocumentService, 'getDocument');
    const getNumSpy = jest.spyOn(DocumentNumberService, 'getUniqueDocumentNumber');

    await StorageDocumentService.cloneStorageDocument(originalDocNumber, defaultUser, defaultContact, false, false);
