...
class ApplicationConfig {
  ...
  loadProperties() {
    ...
    this._maximumConcurrentDrafts = Number.parseInt(process.env.MAXIMUM_CONCURRENT_DRAFTS);
    this._maximumLandingsForOnlineValidation = Number.parseInt(process.env.MAXIMUM_LANDINGS_FOR_ONLINE_VALIDATION, 10);
    this._landingLimitDaysInTheFuture = Number.parseInt(process.env.LANDING_LIMIT_DAYS_IN_THE_FUTURE, 10);
    ...
    this._maximumFavouritesPerUser = Number.parseInt(process.env.MAXIMUM_FAVOURITES_PER_USER, 10);
    ...
    this._maxLimitLandings = Number.parseInt(process.env.LIMIT_ADD_LANDINGS, 10);
    this._maxUploadFileSize = Number.parseInt(process.env.MAX_UPLOAD_FILE_SIZE, 10)
    ...
    this._maxAuthRetries = Number.parseInt(process.env.MAX_AUTH_RETRIES, 10) || 2;
