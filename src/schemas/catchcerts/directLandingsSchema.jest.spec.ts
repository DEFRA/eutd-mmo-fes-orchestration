...
// Set test environment variable for landing limit
if (!process.env.LANDING_LIMIT_DAYS_IN_THE_FUTURE) {
  process.env.LANDING_LIMIT_DAYS_IN_THE_FUTURE = '7'; // 7 days default for tests
  // Reinitialize config to pick up the env variable
...
describe('directLandingsSchema - dateLanded validation', () => {
  ...
  it('returns date.max error when dateLanded exceeds future limit', () => {
    // Skip test if landing limit not configured in test environment
