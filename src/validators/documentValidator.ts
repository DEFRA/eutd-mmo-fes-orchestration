...
export const getProductsCatchCertificate = (productsArray: CatchCertSchema.Product[]) => Array.isArray(productsArray) ? productsArray?.reduce((productDrafts: IProductDraft[], product: CatchCertSchema.Product) => {
    ...
    if (index >= 0) {
      productDrafts[index].totalWeight += product.caughtBy.reduce((totalLandingWeight: number, landing: CatchCertSchema.Catch) => Number.isNaN(landing.weight) ? totalLandingWeight : totalLandingWeight + landing.weight, 0);
      return productDrafts;
    }
    return [...productDrafts, {
      species: product.species,
      speciesCode: product.speciesCode,
      totalWeight: product.caughtBy.reduce((totalLandingWeight: number, landing: CatchCertSchema.Catch) => Number.isNaN(landing.weight) ? totalLandingWeight : totalLandingWeight + landing.weight, 0)
    }]
  }, []) : []
