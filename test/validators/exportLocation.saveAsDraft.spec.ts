...
test('saveAsDraft validation - pointOfDestination should accept empty string', (t) => {
  ...
  t.equals(typeof value.pointOfDestination, 'string', 'pointOfDestination is a string');
  t.end();
});
...
test('saveAsDraft validation - pointOfDestination should accept null', (t) => {
  ...
  t.equals(value.pointOfDestination, null, 'null value is properly validated');
  t.end();
});
...
test('saveAsDraft validation - pointOfDestination should be optional (missing field)', (t) => {
  ...
  t.ok(!error, 'validation passes when field is optional and missing');
  t.end();
});
...
test('saveAsDraft validation - pointOfDestination with ONLY alphanumeric and hyphens SHOULD be cleared', (t) => {
  ...
  t.notEquals(payload.pointOfDestination, '', 'original input was not empty');
  t.end();
});
...
test('saveAsDraft validation - pointOfDestination exceeding 100 characters should fail', (t) => {
  ...
  if (error) {
    ...
    t.ok(error.message.includes('must be less than or equal to'), 'error message indicates max length violation');
  }
  t.end();
});
...
test('saveAsDraft validation - pointOfDestination at 100 character boundary should pass', (t) => {
  ...
  t.equals(longValue.length, 100, 'input is exactly 100 characters');
  t.end();
});
...
test('saveAsDraft validation - pointOfDestination with mixed valid characters should NOT be cleared', (t) => {
  ...
  t.ok(value.pointOfDestination.includes(' '), 'value contains space character');
  t.end();
});
...
test('saveAsDraft validation - comparison: alphanumeric only vs alphanumeric with space', (t) => {
  ...
  t.notEquals(result1.value.pointOfDestination, result2.value.pointOfDestination, 'clearing behavior differs between with/without space');
  t.end();
});
