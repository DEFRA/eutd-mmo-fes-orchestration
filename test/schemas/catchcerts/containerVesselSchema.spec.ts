...
describe('containerVesselSchema - containerNumbers uniqueness validation', () => {
  const validPayload = {
    ...basePayload,
    arrival: true,
    pointOfDestination: 'Port of Le Havre',
  ...
  it('passes validation when all container numbers are unique', () => {
    const payload = { ...validPayload, containerNumbers: ['ABCU1234567', 'MSCU7654321'] };
    const { error } = containerVesselSchemaDefault.validate(payload, { abortEarly: false });
    expect(error).toBeUndefined();
  });

  it('passes validation with a single container number', () => {
    const payload = { ...validPayload, containerNumbers: ['ABCU1234567'] };
    const { error } = containerVesselSchemaDefault.validate(payload, { abortEarly: false });
    expect(error).toBeUndefined();
  });

  it('passes validation when containerNumbers is absent', () => {
    const { error } = containerVesselSchemaDefault.validate(validPayload, { abortEarly: false });
    expect(error).toBeUndefined();
  });

  it('fails validation when containerNumbers contains duplicate values', () => {
    const payload = { ...validPayload, containerNumbers: ['ABCU1234567', 'ABCU1234567'] };
    const { error } = containerVesselSchemaDefault.validate(payload, { abortEarly: false });
    expect(error).toBeDefined();
    const uniqueErr = error!.details.find((d: any) => d.type === 'array.unique');
    expect(uniqueErr).toBeDefined();
    expect(uniqueErr!.context!.pos).toBe(1);
    expect(uniqueErr!.context!.dupePos).toBe(0);
  });

  it('fails validation when first and third container numbers are duplicates', () => {
    const payload = { ...validPayload, containerNumbers: ['ABCU1234567', 'MSCU7654321', 'ABCU1234567'] };
    const { error } = containerVesselSchemaDefault.validate(payload, { abortEarly: false });
    expect(error).toBeDefined();
    const uniqueErr = error!.details.find((d: any) => d.type === 'array.unique');
    expect(uniqueErr).toBeDefined();
    expect(uniqueErr!.context!.pos).toBe(2);
    expect(uniqueErr!.context!.dupePos).toBe(0);
  });

  it('does not flag multiple empty strings as duplicates', () => {
    const payload = { ...validPayload, containerNumbers: ['', ''] };
    const { error } = containerVesselSchemaDefault.validate(payload, { abortEarly: false });
    const uniqueErr = error?.details?.find((d: any) => d.type === 'array.unique');
    expect(uniqueErr).toBeUndefined();
  });

  it('does not flag a non-empty string as a duplicate of an empty string', () => {
    const payload = { ...validPayload, containerNumbers: ['ABCU1234567', ''] };
    const { error } = containerVesselSchemaDefault.validate(payload, { abortEarly: false });
    const uniqueErr = error?.details?.find((d: any) => d.type === 'array.unique');
    expect(uniqueErr).toBeUndefined();
