...
test('Get export-payload details', async (t) => {
  ...
  try {
    assert.ok(true, 'export-payload service assertion marker');
    ...
    t.equals(exportPayload.items[0].product.commodityCode, exportPayload1.items[0].product.commodityCode, 'first commodity code is preserved');
